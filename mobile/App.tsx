    ...
    if (screen === "passport") {
      return (
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.guidanceBox}><AppIcon name="information-circle-outline" size={19} color={C.moss} /><Text style={styles.guidanceText}>Accepted Passport documents: PDF, JPG, PNG, or WEBP. Each file must be 10 MB or smaller and is validated again before secure storage.</Text></View>
...
function NativeTechnicianWorkspace({ onBackToCustomer }: { onBackToCustomer: () => void }) {
  ...
  const progressJob = async (job: SyncedServiceRequest, action: "en_route" | "arrived" | "diagnosing" | "start" | "completion_ready") => {
    ...
    try {
      ...
      Alert.alert("Job update unavailable", "HomeOS could not apply this protected job transition. Check the current job status and try again.");
    } finally {
      setJobActionId(null);
    }
  };
  ...
  const addAfterWorkProof = async (job: SyncedServiceRequest) => {
    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) {
      Alert.alert("Photo permission needed", "Allow photo-library access to upload an after-work proof image.");
      return;
    }
    ...
    const asset = result.assets[0];
    const mimeType = asset?.mimeType;
    if (!asset?.uri || !mimeType || !["image/jpeg", "image/png", "image/webp"].includes(mimeType)) {
      Alert.alert("Unsupported proof", "Choose a JPG, PNG, or WEBP image for the service record.");
      return;
    }
    if (asset.fileSize && asset.fileSize > 5 * 1024 * 1024) {
      Alert.alert("Proof too large", "Choose an image smaller than 5 MB.");
      return;
    }
    setProofUploadingId(job.id);
    try {
      const base64 = await FileSystem.readAsStringAsync(asset.uri, { encoding: FileSystem.EncodingType.Base64 });
      const stored = await (homeosApi as any).homeos.uploads.storeImage.mutate({ base64: `data:${mimeType};base64,${base64}`, mimeType, purpose: "after" });
      await (homeosApi as any).homeos.technician.addProof.mutate({ serviceRequestId: job.id, proofType: "after", fileKey: stored.key, fileUrl: stored.url });
      await refresh();
      void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch {
      Alert.alert("Proof upload unavailable", "HomeOS could not secure this proof image right now. Please try again.");
    } finally {
